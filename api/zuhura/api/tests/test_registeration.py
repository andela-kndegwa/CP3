...
class TestUserRegisteration(APITestCase):
    ...
    def test_user_registeration(self):
      '''
      Arguments:
          user name, email and password as in the setUp
          class.
      ...
      Asserts:
          User can be created appropriately.
      '''
      self.assertEquals(
    ...
    def test_users_increase_after_registration(self):
      '''
      Asserts:
          Confirms Users increase by one after registeration.

      '''
      # The users before value is 0 since there are no Users 
      # registration.
      users_after = User.objects.all().count()
      self.assertGreater(users_after, self.users_before)
      self.assertEqual(0, self.users_before)
      self.assertEqual(1, users_after)
